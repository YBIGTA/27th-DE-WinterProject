...
core_files:
  - connectors/s3-sink-config.json
...
5.  **파티셔닝:** `TimeBasedPartitioner`를 사용하여 서버의 현재 시간(`Wallclock`)을 기준으로 `year=YYYY/month=MM/day=dd` 형식의 디렉터리 구조를 만들어 파일을 저장합니다.
...
| **Time-based Partitioner 사용** | 데이터를 시간 순서(연/월/일)로 S3에 정리하면 향후 분석 및 조회가 용이해집니다.                                                      | 이벤트 자체의 시간 필드가 아닌 커넥터 서버의 시간(`Wallclock`)을 사용하므로, 데이터 처리 지연이 심할 경우 이벤트 발생 시각과 다른 날짜에 데이터가 저장될 수 있습니다. |
