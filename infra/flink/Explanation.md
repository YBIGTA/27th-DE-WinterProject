...
## Role
Kafka로부터 유입되는 원시 택시 이벤트를 수집하여 **순서 재정렬(Reordering)** 후 ClickHouse에 이벤트를 적재하고, 동시에 3분 단위 수요 집계 기반 ONNX 추론 결과를 `taxi_predictions`로 적재하는 스트리밍 컴포넌트입니다.
...
flowchart TD
    ...
    P --> Q[ONNX Inference]
    Q --> R[JDBC Sink: taxi_predictions]
```
