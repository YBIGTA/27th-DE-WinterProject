...
[Generator] --(HTTP POST /ingest, /ingest/batch)--> [Nginx LB] --(HTTP proxy_pass + random two least_conn)--> [Ingestor Cluster]
...
## 3. 분산 라우팅: random two least_conn
...
- 라우팅 정책은 `random two least_conn`입니다.
  - 두 개를 랜덤 샘플링한 뒤 연결 수가 더 적은 upstream을 선택합니다.
- upstream keepalive 풀(`keepalive 64`)은 기존 TCP 연결을 재사용하기 위한 설정입니다.
...
## 5. 실패 처리: request-level retry + next upstream
...
- Distributed 템플릿:
  - `location /health`는 Nginx가 직접 `200 OK`를 반환합니다.
  - 마찬가지로 upstream ingestor 상태와는 분리됩니다.

## 7. 전체 흐름을 한 줄로 요약
1. Generator 요청이 Nginx LB로 들어온다.  
2. Nginx가 `random two least_conn`으로 ingestor를 선택한다.  
3. 프록시 타임아웃/버퍼 정책으로 요청을 전달한다.  
4. 실패 시 `proxy_next_upstream` 규칙으로 다른 ingestor로 재시도한다(최대 2회).  
...
2. URI 매칭  
   single-machine/distributed 모두 `/health`와 그 외 경로를 분기 처리.
3. 업스트림 후보 선택  
   `random two least_conn`으로 ingestor 후보 결정.
4. 백엔드 연결 확보  
...
| 3 | Nginx upstream selector | upstream peer 후보 목록 | `random two least_conn` 선택 결과 |
...
2. 업스트림 선택 정책  
   `random two least_conn`으로 과부하 인스턴스를 상대적으로 회피합니다.
3. 재시도 정책  
   어떤 실패를 다른 peer로 넘길지(`proxy_next_upstream`, `proxy_next_upstream_tries=2`)를 결정합니다.
4. 프록시 버퍼링  
...
- 장애 대응은 active probe가 아니라 요청 단위 재시도(`proxy_next_upstream`) 기반이며, 실패 시 조건이 맞으면 다른 upstream으로 재시도합니다.
- `/health`는 single-machine/distributed 모두에서 Nginx 자체 생존 체크를 반환하며 upstream 상태와 분리됩니다.
